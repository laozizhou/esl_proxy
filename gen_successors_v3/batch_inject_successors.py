...
def main():
    ...
    for h_file in sorted(root.rglob("*.h")):
        if h_file.stem.endswith("_suc"):
            continue
        try:
            out_path, total_local, total_edges, n_groups, total_task_cnt = process_file(h_file)
            processed.append(h_file)
            print(f"OK   {h_file} -> {out_path}: {total_local}/{total_edges} edges, {n_groups} group(s), total_task_cnt={total_task_cnt}")
