...
// Cross-task ordering is expressed explicitly via add_predecessors() before
// desc_task_id is incremented by DESC_DO_OR_SKIP. SPMD tier is selected at
// compile time via QWEN3_SPMD_TIER (0=non-spmd .. 4=all-spmd).
//
...
int orchestrator_desc(const uint64_t orch_args, int thread_id, int *created_cnt) {
    ...
    /* Predecessor tracking arrays (sized for worst-case SPMD tier 0). */
    uint32_t qk_norm_per_tile[6];
    uint32_t v_ids_per_tile[6][8];
    uint32_t v_cnt_per_tile[6];
    uint32_t os_by_b[90][4];
    uint32_t os_cnt_by_b[90];
    uint32_t batch_predecessors[90]; /* reusable scratch buffer */
    ...
    for (int64_t b0 = 0; b0 < batch_padded; b0 += 16) {
        ...
        /* ── RMS Norm ── */
        const uint32_t rmsnorm_id = (uint32_t)desc_task_id;
        ...
        /* ── Q Proj (depends on RMS Norm) ── */
        uint32_t q_ids[20];
        int qi = 0;
        for (int base = 0; base < 20; base += qwen3_blocks_per_task(20)) {
            q_ids[qi] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id, {
        ...
        for (int base = 0; base < 8; base += qwen3_blocks_per_task(8)) {
            ...
            k_ids[ki] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
            ...
            v_ids[vi] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
        ...
        /* ── QK Norm (depends on all Q-proj + K-proj tasks) ── */
        qk_norm_per_tile[tix] = (uint32_t)desc_task_id;
        DESC_DO_OR_SKIP(desc_task_id,  {
            ...
            int idx = add_predecessors(__did, q_ids, (uint32_t)qwen3_n_tasks(20, qwen3_blocks_per_task(20)), 0);
            add_predecessors(__did, k_ids, (uint32_t)qwen3_n_tasks(8, qwen3_blocks_per_task(8)), idx);
        });
    }
    ...
    for (int64_t b = 0; b < user_batch; b += 1) {
        ...
        /* ── ROPE KV Cache (depends on QK Norm + V-proj for this tile) ── */
        const uint32_t rope_id = (uint32_t)desc_task_id;
        {
            DESC_DO_OR_SKIP(desc_task_id,  {
                ...
                int tmp = add_predecessors(__did, batch_predecessors, 1, 0);
                add_predecessors(__did, v_ids_per_tile[tix], v_cnt_per_tile[tix], tmp);
            });
        }

        /* ── Per-chunk attention sub-tasks ── */
        uint32_t qk_ids[4];
        uint32_t sm_ids[4];
        uint32_t sv_ids[4];
        int ci = 0;
        for (int base = 0; base < 4; base += qwen3_blocks_per_task(4)) {
            ...
            /* QK Matmul (depends on ROPE) */
            qk_ids[ci] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
            ...
            sm_ids[ci] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
            ...
            /* Online Softmax (depends on SV Matmul + Softmax) */
            os_by_b[b][ci] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
                Tensor oi_tmp_in = view(all_oi_tmp, base * 1024u, 0u, (uint32_t)(cur_blocks * 1024), 128u);
        ...
        os_cnt_by_b[b] = (uint32_t)qwen3_n_tasks(4, qwen3_blocks_per_task(4));
    }
    ...
    for (int64_t b0 = 0; b0 < batch_padded; b0 += 16) {
        ...
        /* ── Out Proj (depends on all Online-Softmax tasks for the rows in this tile) ── */
        uint32_t op_ids[40];
        int opi = 0;
        for (int base = 0; base < 40; base += qwen3_blocks_per_task(40)) {
            op_ids[opi] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
                ...
                for (int64_t row = 0; row < cur_valid; row++) {
                    ...
                    idx = add_predecessors(__did, os_by_b[bb], os_cnt_by_b[bb], idx);
                }
            });
            opi++;
        }

        /* ── Post RMSNorm (depends on all Out-Proj tasks) ── */
        const uint32_t post_rmsnorm_id = (uint32_t)desc_task_id;
        DESC_DO_OR_SKIP(desc_task_id,  {
        ...
        /* ── Gate Proj / Up Proj / SILU (depend on Post RMSNorm) ── */
        ...
        for (int base = 0; base < 34; base += qwen3_blocks_per_task(34)) {
            ...
            gate_ids[gi] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
            ...
            up_ids[ui] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
            ...
            silu_ids[si] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
        ...
        /* ── Down Proj + Down-Proj-Res (depends on SILU + Out-Proj) ── */
        ...
        for (int base = 0; base < 40; base += qwen3_blocks_per_task(40)) {
            ...
            down_ids[di] = (uint32_t)desc_task_id;
            DESC_DO_OR_SKIP(desc_task_id,  {
